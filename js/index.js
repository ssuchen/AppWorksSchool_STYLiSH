...
function ajaxLoading(type,textpage){
    let linkLoading = "https://api.appworks-school.tw/api/1.0/products/"+ type +"?paging=" +textpage
    return linkLoading              
};
...
function imgrender(data){
    ...
    let bgLink = document.createElement('div');
        bgLink.setAttribute('style','background-image:url("'+ Photo +'");width:100%;height:100%;' );
...
function campaignsLink(){
    let campaignslinke = "https://api.appworks-school.tw/api/1.0/marketing/campaigns";
...
//========預設抓取全部 連結位置==============================================================
    ...
    //console.log(getOtherUrlString)
  
    if( getOtherUrlString == "women" || getOtherUrlString == "men" || getOtherUrlString == "accessories"){
...
//  //女裝
//  let WomenClothingBtn = document.querySelector('.Women-clothing');
//  //WomenClothingBtn.addEventListener('click',WomenPage,false);
...
//    ajax(ajaxLine("women"), function(response)
//    { render(response);
...
// //男裝
// let MenclothingBtn = document.querySelector('.Men-clothing');
//     //MenclothingBtn.addEventListener('click',MenPage,false);
// function MenPage(){
//         Clothingtype = "men";
//         //重新放上新標籤  
//         let text = document.querySelector('.index-content-width');
...
//         ajax(ajaxLine("men"), function(response)
//         { render(response);
...
//    // AccessoriesBtn.addEventListener('click',accessoriesPage,false);
...
//     ajax(ajaxLine("accessories"), function(response)
//     { render(response);
